...
const ABAN_CALLBACK_URL = String(process.env.ABAN_CALLBACK_URL || `${BACKEND_PUBLIC_URL}/api/orders/payment/callback`).trim();

async function abanRequest(path, options = {}) {
    if (!ABAN_API_TOKEN) {
        throw new Error("ABAN_API_TOKEN در تنظیمات سرور وجود ندارد.");
    }
    ...
    if (!response.ok) {
        const message = payload?.message || payload?.error?.message || payload?.error || `Aban Gateway HTTP ${response.status}`;
...
async function createAbanInvoice({ orderCode, totalToman }) {
    ...
    const callbackUrl = `${ABAN_CALLBACK_URL}${ABAN_CALLBACK_URL.includes("?") ? "&" : "?"}order_id=${encodeURIComponent(orderCode)}`;
    ...
    try {
        ...
        try {
            ...
            if (!invoiceId || !paymentUrl) {
                ...
                throw new Error("Aban لینک پرداخت را برنگرداند.");
            }
            ...
            return res.status(paymentError.status === 422 ? 422 : 502).json({
                success: false,
...
// Aban returns the customer to this public callback URL after payment.
// The callback itself is never trusted: payment is confirmed by Aban's verify API.
    ...
    try {
        ...
        try {
            ...
            if (verifyError.status === 409) {
                const { data: current } = await supabase.from("orders").select("*").eq("order_code", order.order_code).single();
                return res.json({ success: true, verified: true, alreadyVerified: true, order: mapOrder(current) });
            }
            throw verifyError;
        }
        ...
        return res.json({ success: true, verified: true, order: mapOrder(paidOrder) });
    } catch (error) {
