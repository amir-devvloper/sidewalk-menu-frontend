...
// Rebuilds the "مبلغ سفارش" box from checkoutSubtotal + appliedDiscount.
// Called once when checkout opens and again every time a discount code is
// successfully applied or cleared.
function renderCheckoutTotal() {
    const box = document.getElementById("checkoutTotalBox");
    if (!box) return;

    if (!appliedDiscount) {
        box.classList.remove("checkout-total-multi");
        box.innerHTML = `
    ...
    box.classList.add("checkout-total-multi");
    box.innerHTML = `
        <div class="checkout-total-row">
            <span>مبلغ سفارش</span>
            <span>${formatPrice(checkoutSubtotal)} ${TOMAN_SVG}</span>
        </div>

        <div class="checkout-total-row discount-row">
            <span>تخفیف (${appliedDiscount.discountPercent}%)</span>
            <span>- ${formatPrice(appliedDiscount.discountAmount)} ${TOMAN_SVG}</span>
        </div>

        <div class="checkout-total-row checkout-total-final">
            <span>مبلغ نهایی</span>
            <strong>${formatPrice(appliedDiscount.finalTotal)} ${TOMAN_SVG}</strong>
        </div>
    `;
}
...
async function applyDiscountCode() {
    ...
    if (!code) {
        showToast("لطفاً یک کد تخفیف وارد کنید.");
        return;
    }
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/orders/discount/validate`, {
            method: "POST",
        ...
        if (!response.ok || !data.success) {
            ...
            if (resultBox) {
                resultBox.textContent = data.message || "کد تخفیف نامعتبر است.";
                resultBox.className = "discount-result discount-error";
            }
            return;
        }
        ...
        if (resultBox) {
            resultBox.textContent = `کد «${data.discount.code}» با موفقیت اعمال شد.`;
            resultBox.className = "discount-result discount-success";
        }
    } catch (error) {
        console.error("Discount validate error:", error);
        showToast("خطا در برقراری ارتباط با سرور. دوباره تلاش کنید.");
    } finally {
        if (button) {
            button.disabled = false;
            button.textContent = "اعمال";
        }
    }
}
...
function openCheckout() {
    ...
    const applyDiscountBtn =
        document.getElementById("applyDiscountBtn");
    ...
    if (discountInput) {
        ...
        // Typing again after a code was already applied means the
        // customer wants to try a different one — clear the stale result
        // instead of silently keeping the old discount applied.
        discountInput.addEventListener("input", () => {
            ...
            if (resultBox) {
                ...
                resultBox.className = "discount-result";
            }
        });
    }
