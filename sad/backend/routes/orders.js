...
const ABAN_CALLBACK_URL =
    String(
        process.env.ABAN_CALLBACK_URL ||
        "https://sidewalk-menu-backend.onrender.com/api/orders/payment/callback"
    ).trim();

function getFrontendRedirectUrl(status, orderCode) {
    const origins = String(process.env.FRONTEND_ORIGINS || "")
        .split(",")
        .map(value => value.trim())
        .filter(Boolean);

    const origin = origins[0] || "https://sidewalk-menu-frontend.pages.dev";
...
async function abanRequest(path, options = {}) {
    if (!ABAN_API_TOKEN) {
        const error = new Error("درگاه پرداخت روی سرور تنظیم نشده است.");
        error.status = 503;
        throw error;
    }
    ...
    try {
        ...
        // Keep the HTTP status as the useful error when Aban returns no JSON.
    }
...
async function createAbanInvoice({ orderCode, totalToman, customerName, customerPhone }) {
    ...
    if (!Number.isSafeInteger(amountRial) || amountRial <= 0) {
        const error = new Error("مبلغ سفارش برای پرداخت معتبر نیست.");
        error.status = 400;
        throw error;
    }
...
function mapOrder(order, { publicView = false } = {}) {
    ...
    return {
        ...
        paymentInvoiceId: order.payment_invoice_id || null,
...
router.post("/", async (req, res) => {
    try {
        ...
        // Create the payment invoice on the server. The browser never receives
        // the Aban API token and the amount is calculated from DB prices above.
        try {
            ...
            if (!invoiceId || !paymentUrl) {
                throw new Error("درگاه پرداخت لینک پرداخت معتبر برنگرداند.");
            }

            const { data: updatedOrder, error: paymentUpdateError } = await supabase
                .from("orders")
                .update({
            ...
            if (paymentUpdateError || !updatedOrder) {
                console.error("Payment data update error:", paymentUpdateError?.message);
                throw new Error("ذخیره اطلاعات پرداخت سفارش ناموفق بود.");
            }
            ...
            console.error("Aban invoice error:", paymentError?.message);

            // The order was never paid. Remove this newly-created order so
            // a failed invoice creation does not leave a dead unpaid order.
            ...
            if (cleanupError) {
                console.error("Unpaid order cleanup error:", cleanupError.message);
            }

            return res.status(paymentError?.status === 503 ? 503 : 502).json({
                success: false,
...
// Aban sends the customer back here after payment.
// Verification is done server-to-server; the callback itself is never trusted.
async function handleAbanCallback(req, res) {
    const invoiceId =
    ...
    try {
        ...
        if (invoiceId) {
            const { data, error } = await supabase
                .from("orders")
                .select("*")
                .eq("payment_invoice_id", String(invoiceId))
                .maybeSingle();
        ...
        if (!order || !order.payment_invoice_id) {
            return res.redirect(getFrontendRedirectUrl("failed"));
        }

        const actualInvoiceId = String(order.payment_invoice_id);

        // The verify endpoint is the authoritative payment confirmation.
        // A 409 means this invoice was already verified, which is also safe.
        try {
            await verifyAbanInvoice(actualInvoiceId);
        } catch (verifyError) {
            if (verifyError?.status !== 409) {
                console.error("Aban callback verify error:", verifyError?.message);
                return res.redirect(
                    getFrontendRedirectUrl("failed", order.order_code)
                );
            }
        }
        ...
        return res.redirect(
            getFrontendRedirectUrl("success", order.order_code)
        );
    } catch (error) {
        console.error("Aban callback error:", error?.message);
        return res.redirect(
            getFrontendRedirectUrl("failed", orderCode)
        );
    }
}

router.get("/payment/callback", handleAbanCallback);
router.post("/payment/callback", handleAbanCallback);

// Optional frontend fallback: it can ask the backend to verify a known invoice.
router.get("/payment/verify/:invoiceId", async (req, res) => {
    const invoiceId = String(req.params.invoiceId || "").trim();
    if (!invoiceId) {
        return res.status(400).json({ success: false, message: "شناسه پرداخت نامعتبر است." });
    }
    ...
    try {
        ...
        return res.status(error?.status || 500).json({
            success: false,
            message: error?.message || "تأیید پرداخت ناموفق بود."
        });
    }
});
