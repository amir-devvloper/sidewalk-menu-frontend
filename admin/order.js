...
//   cancelled -> "پرداخت لغو شد"   (این با «لغو شد»ی که مال وضعیت سفارشه فرق داره)
...
const discountsEmptyState = document.getElementById("discountsEmptyState");
const discountsLoadingState = document.getElementById("discountsLoadingState");
...
const df_code = document.getElementById("df_code");
const df_discountPercent = document.getElementById("df_discountPercent");
const df_minOrderAmount = document.getElementById("df_minOrderAmount");
const df_startsAt = document.getElementById("df_startsAt");
const df_expiresAt = document.getElementById("df_expiresAt");
const df_isActive = document.getElementById("df_isActive");
...
document.querySelectorAll(".nav-item").forEach(item => {
    item.addEventListener("click", event => {
        ...
        if (view === "discounts" && discounts.length === 0) {
            loadDiscounts();
        }
...
const DISCOUNT_STATUS_CLASSES = {
    active: "status-ready",
    scheduled: "status-payment-pending",
...
async function loadDiscounts() {
    discountsLoadingState.classList.remove("hidden");
    discountsEmptyState.classList.add("hidden");

    try {
        const response = await authFetch(`${API_BASE}/admin/discounts`);
        ...
        console.error(error);
        if (error.message !== "Unauthorized") {
            showToast("دریافت کدهای تخفیف با مشکل مواجه شد.");
        }
    } finally {
        discountsLoadingState.classList.add("hidden");
    }
}
...
function renderDiscounts() {
    ...
    if (discounts.length === 0) {
        discountsEmptyState.classList.remove("hidden");
        return;
    }
    ...
    discounts.forEach(discount => {
        discountsContainer.insertAdjacentHTML("beforeend", createDiscountCard(discount));
    });
}
...
function createDiscountCard(discount) {
    ...
    return `
        <article class="product-card discount-card" data-discount="${escapeHTML(discount.id)}">
            <div class="product-top">
                <strong class="discount-code-label">${escapeHTML(discount.code)}</strong>
                <span class="status-badge ${statusClass}">${escapeHTML(statusLabel)}</span>
            </div>

            <div class="discount-details">
                <div><i class="fa-solid fa-percent"></i> ${discount.discountPercent}% تخفیف</div>
                <div><i class="fa-solid fa-sack-dollar"></i> حداقل سفارش: ${formatPrice(discount.minOrderAmount)} تومان</div>
                <div><i class="fa-regular fa-clock"></i> شروع: ${startsAtLabel}</div>
                <div><i class="fa-regular fa-clock"></i> پایان: ${expiresAtLabel}</div>
            </div>

            <div class="order-actions">
                <button class="action-btn" onclick="editDiscount('${escapeJS(discount.id)}')">ویرایش</button>
                <button
                    class="action-btn ${discount.isActive ? "delete-btn" : ""}"
                    onclick="toggleDiscountActive('${escapeJS(discount.id)}', ${!discount.isActive})"
                >
                    ${discount.isActive ? "غیرفعال کردن" : "فعال کردن"}
                </button>
                <button class="action-btn delete-btn" onclick="deleteDiscount('${escapeJS(discount.id)}')">حذف</button>
            </div>
        </article>
    `;
}

async function toggleDiscountActive(id, isActive) {
    try {
        const response = await authFetch(
            `${API_BASE}/admin/discounts/${encodeURIComponent(id)}`,
            {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
        ...
        if (!response.ok) {
            const data = await response.json().catch(() => ({}));
            throw new Error(data.message || "تغییر وضعیت با خطا مواجه شد.");
        }

        const updated = await response.json();
        const index = discounts.findIndex(d => d.id === id);
        if (index !== -1) discounts[index] = updated;

        renderDiscounts();
        showToast(isActive ? "کد تخفیف فعال شد." : "کد تخفیف غیرفعال شد.");
...
addDiscountBtn.addEventListener("click", () => {
    ...
    discountFormTitle.textContent = "کد تخفیف جدید";
    discountForm.reset();
    df_isActive.checked = true;
    df_minOrderAmount.value = 0;
...
function editDiscount(id) {
    ...
    discountFormTitle.textContent = "ویرایش کد تخفیف";
...
discountCancelBtn.addEventListener("click", () => {
...
discountForm.addEventListener("submit", async event => {
    ...
    if (new Date(expiresAt) <= new Date(startsAt)) {
        showToast("تاریخ پایان باید بعد از تاریخ شروع باشد.");
        return;
    }

    const payload = {
        code: df_code.value.trim().toUpperCase(),
        discountPercent: Number(df_discountPercent.value),
        minOrderAmount: Number(df_minOrderAmount.value || 0),
        startsAt,
        expiresAt,
        isActive: df_isActive.checked
    };

    try {
        const url = editingDiscountId
            ? `${API_BASE}/admin/discounts/${encodeURIComponent(editingDiscountId)}`
        ...
        if (!response.ok) {
            const data = await response.json().catch(() => ({}));
            throw new Error(data.message || "ذخیره کد تخفیف با خطا مواجه شد.");
        }

        discountModal.classList.add("hidden");
        showToast("کد تخفیف با موفقیت ذخیره شد.");
        loadDiscounts();
...
async function deleteDiscount(id) {
    const confirmed = confirm("آیا از حذف این کد تخفیف مطمئن هستید؟");
    if (!confirmed) return;
    ...
    try {
        ...
        if (!response.ok) {
            const data = await response.json().catch(() => ({}));
            throw new Error(data.message || "حذف کد تخفیف با خطا مواجه شد.");
        }
